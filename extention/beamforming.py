...
def mcmf_wrapper_on_masks(mix, noise_mask=None, target_mask=None,
                         setup=None, corr=None, expand_frames=1): 
    mix = mix.T
    if noise_mask is not None:
        noise_mask = noise_mask.T
    if target_mask is not None:
    ...
    if setup is None:
        ...
        output_type = setup['output_type']
        mu = setup['mwf_mu']
        expand_frames = setup['expand_frames']
        
    # change the mix to multi-frame version
    bins, sensors, _ = mix.shape
    mix_expand = mix
    for l in range(1,expand_frames):
        tmp0 = np.zeros((bins,sensors,l), dtype=np.complex)
        tmp1 = mix[:,:,:-l]
        tmp = np.concatenate((tmp0, tmp1), axis=2)
        mix_expand = np.concatenate((mix_expand, tmp), axis=1)
    mix = mix_expand
    
    # change the mask values
    target_mask_expand = np.tile(target_mask[:,np.newaxis,:], (1,sensors,1))
    noise_mask_expand = np.tile(noise_mask[:,np.newaxis,:], (1,sensors,1)) 
    for l in range(1,expand_frames):
        tmp0 = np.zeros((bins,sensors,l), dtype=np.float32)
        tmp1 = target_mask_expand[:,:sensors,:-l]
        tmp = np.concatenate((tmp0, tmp1), axis=2)
        target_mask_expand = np.concatenate((target_mask_expand, tmp), axis=1)
        tmp1 = noise_mask_expand[:,:sensors,:-l]
        tmp = np.concatenate((tmp0, tmp1), axis=2)
        noise_mask_expand = np.concatenate((noise_mask_expand, tmp), axis=1)
    
    target_psd_matrix = get_power_spectral_density_matrix(mix, target_mask_expand, expand_frames)
    noise_psd_matrix = get_power_spectral_density_matrix(mix, noise_mask_expand, expand_frames)
    ...
    if output_type == 'vs':
		# variable span linear filter
		# ref: noise reduction with optimal variable span linear filters
		output = apply_vs_filter(mix, target_psd_matrix, noise_psd_matrix, mu, corr, expand_frames)
